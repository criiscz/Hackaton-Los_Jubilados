...
const EXTRA_MOODS = [
  { id: 'nature',  label: 'Nature',  items: ['🦋', '🌻', '🐚', '🍒', '🍑', '🌶️', '🐝', '🌙', '🌿', '🪻'] },
  { id: 'symbol',  label: 'Symbols', items: ['🪩', '✨', '🛸', '👾', '🎀', '🍭', '🪐', '⚡', '🩰', '🧿'] },
  { id: 'animals', label: 'Animals', items: ['🐍', '🦊', '🐯', '🦄', '🦦', '🐙', '🦩', '🐉', '🐺', '🦝'] },
  { id: 'food',    label: 'Food',    items: ['🍓', '🍷', '🍯', '🥑', '🥥', '🌽', '🧁', '🍩', '🥮', '🍙'] },
];

const GROUPS = [
  { id: 'soft',  label: 'Soft',    items: SAFE_EMOJIS.map((e) => e.char) },
  { id: 'spicy', label: 'Spicy 🌶', items: SPICY_EMOJIS.map((e) => e.char) },
  ...EXTRA_MOODS,
];
...
export default function EmojiPicker({ value, onChange, onClose, open }) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Pick your signature emoji"
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <h3 className="display-italic text-[26px] text-ink leading-tight">
              Pick <span className="text-pink">your emoji</span>
            </h3>
          </div>
          <button
            type="button"
